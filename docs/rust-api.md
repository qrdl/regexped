...
> **Component format:** `stub_type: rust` works under `wasm_format: component` too, and the **public API is identical** — same iterators, same `Error`/`Result`, same `Span`, same named-group constants — so switching formats changes no calling code. What differs is underneath: the stub carries a `wit_bindgen::generate!` invocation instead of `#[link]` blocks, your crate gains a `wit-bindgen` dependency, you build for `wasm32-wasip2`, and `regexped merge` composes with `wac` rather than merging with `wasm-merge` (the command you type is the same). See [component.md](component.md) and `examples/wasmtime/rust/secrets`.
...
Under `wasm_format: component` the API above is unchanged — same iterators, same
`Error`/`Result`, same `Span`, same named-group constants. Only the build differs.

```sh
regexped compile      # regexps.wasm (a component) + regexps.wit
regexped generate     # stubs.rs
cargo build --release --target wasm32-wasip2
regexped merge --config=regexped.yaml --main=target/wasm32-wasip2/release/your.wasm regexps.wasm
...
That dependency is not a convenience. A component consumer cannot declare its
imports by hand: `wasm32-wasip2` wraps into a component at link time and requires
component-type metadata that only a binding generator embeds, so the generated
stub opens with `wit_bindgen::generate!` and hides it behind the familiar API. The
WIT is passed **inline**, so there is no `wit/` directory to manage.

Until the merge step runs, your component has an unsatisfied import and will not
instantiate.

The stub is included the same way as the module-format one, and it does not care
where:

```rust
include!("stubs.rs");                 // at the crate root
mod stubs { include!("stubs.rs"); }   // or inside a module — both compile
```

`wit_bindgen::generate!` emits its modules wherever it is expanded, and the
wrapper reaches them with a relative path, so the file is position-independent.

### Which output kind to choose

The component format buys language-agnostic composition: your component can be
plugged together with components written by anyone, and it owns its own memory
rather than sharing a merged one. If you are building a self-contained Rust binary
and none of that applies, `wasm_format: module` is one fewer tool and one fewer
dependency.

A **host** — a native binary loading the component rather than being one — needs no
stub at all. Use `wasm_format: component` with `stub_type` unset and bind against
the generated `.wit` with `wasmtime::component::bindgen!`. See
