...
- **Pattern sets** — compile multiple patterns into a single merged DFA and declare which of seven questions you need answered (`match`/`match_any`/`match_all` anchored, `scan`/`scan_any`/`scan_all` non-anchored, `find` for positions and extents); one call scans for all patterns simultaneously, and `find` returns `(pattern_id, start, end)` tuples; SIMD Teddy (≤16 literals), Aho-Corasick (17+ literals, under a 512 KB table budget), or a density/hint-selected SIMD Shufti prefilter keep per-byte cost near-constant in set size, with a scalar DFA fallback for sets without mandatory literals
