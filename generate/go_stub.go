...
`, pub, pub, s.ScanAll, idKonst)
		...
		if s.Find != "" {
			...
			if sh := overlapCacheShapeFor(s, cfg); sh.Eligible {
				...
				//
				// Allocated lazily beside the gates and reclaimed with the
				// iterator, which is what `init`/`free` are in a language with
				// a collector. The stride is the one field the caller owns,
				// because the caller is what sized the allocation from it, and
