...
// TestByteIdenticalSweepFixturesSweep: a fixture named for the overlapping sweep
// must compile to one. If its shape stopped qualifying, the regenerated bytes
// would pin the walk while the fixture's name still claimed the sweep.
func TestByteIdenticalSweepFixturesSweep(t *testing.T) {
	n := 0
	for _, name := range byteIdentFixtures(t) {
		...
		if !sh.Eligible {
			t.Errorf("%s: the set gets no sweep", name)
		}
	}
	if n == 0 {
		t.Fatal("no fixture is named for the sweep")
	}
}
