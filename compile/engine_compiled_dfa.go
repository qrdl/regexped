...
func buildHybridFindBody(t *dfaTable, l *dfaLayout, mandatoryLit *mandatoryLit, tableMemIdx int, hasTwin, lnm bool) ([]byte, findFromMode, int) {
	return buildFindBody(findBodyParams{
		...
		// measureClassChainFor, not classChainFor: this is the Compiled DFA
		// body, and every pattern small enough to reach the class chain is
		// small enough to be promoted here. Gating only the plain-DFA site
		// left T0.6's A4 row reporting `identical` — the mechanism was emitted
		// and the mask could not reach it.
