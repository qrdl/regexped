...
// All patterns are compiled into one component: secrets.
include!("stubs.rs");
...
fn run() -> secrets::Result<()> {
    ...
    fn report<I>(label: &str, input: &[u8], matches: I, found: &mut bool) -> secrets::Result<()>
    where
        I: Iterator<Item = secrets::Result<(usize, usize)>>,
    {
    ...
    let mut found = false;
    report("GitHub token", input, secrets::find_github_token(input, 0), &mut found)?;
    report("JWT", input, secrets::find_jwt_token(input, 0), &mut found)?;
    report("AWS key", input, secrets::find_aws_key(input, 0), &mut found)?;
