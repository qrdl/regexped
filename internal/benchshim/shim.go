// Package benchshim builds the tiny WASM shim modules the benchmark harnesses
// use to TIME a regexp export from inside wasm, and decodes the samples they
// leave behind.
//
// One copy, not three. tools/perftest/shims.go and tools/likelytest/shims.go
// were byte-identical files (verified: same md5) holding hand-assembled WASM;
// this branch edited the same bytes identically in both, and the Go-side drive
// they feed was then fixed in exactly ONE of the two tools — which is
// an exhaustive-find loop that measured a logarithmic sliver of the work in
// likelytest alone. A shim emitter duplicated by hand is a
// measurement bug waiting for one of its copies to be forgotten.
//
// Each shim:
...
//
//	[0 .. TimingsBytes-1]  timings[Iters] u32 nanosecond samples
...
const (
	// Iters is the number of timing samples a shim records per run. It used to
	// live in each tool's main.go, one copy per tool.
	Iters = 10_000

	TimingsBytes = Iters * 4           // 40 000 bytes
	ClockScratch = int32(TimingsBytes) // 40 000; 8-byte aligned (40000 = 8×5000)
	MemPages     = 1                   // 64KB; enough for 40 008 bytes
)

// ComputeStat reads Iters u32 nanosecond samples from data and returns
...
// shimMemorySection declares 1 memory: min=MemPages pages, no max.
func shimMemorySection() []byte {
	return shimSection(0x05, []byte{0x01, 0x00, byte(MemPages)})
}
