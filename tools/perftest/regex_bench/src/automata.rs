...
struct RaState {
    ...
    /// `find` pairing.
    each: Vec<MetaRegex>,
    /// Reusable PatternSet capacity == pattern count.
    npat: usize,
}
...
/// scan: 1 if any pattern matches at or after `from`, else 0.
#[no_mangle]
pub extern "C" fn ra_scan(len: i32, from: i32) -> i32 {
    let st = state();
    let h = haystack(len);
...
pub extern "C" fn ra_scan_all(len: i32, from: i32) -> i32 {
    ...
    let input = Input::new(h).span(from as usize..h.len());
    let mut ps = PatternSet::new(st.npat);
    st.overlap.which_overlapping_matches(&input, &mut ps);
...
#[no_mangle]
pub extern "C" fn ra_bench_scan(len: i32, iters: i32) {
    let st = state();
    let h = haystack(len);
    timed!(iters, {
        let input = Input::new(std::hint::black_box(h));
        let _ = std::hint::black_box(st.set.is_match(input));
    });
}
