...
│ │ # bitmap row OR'd into the caller's `_all` bitmap ( item
...
│   │                          #   next, dtor. The constructor COPIES the input and DETACHES
│   │                          #   its blocks from the allocator's per-call chain: a resource
...
│   │                          #   construction; the scanner struct's gates[0] carries the
...
│                              #   exports groups, since a returned list is allocated in the
│                              #   CONSUMER's memory
├── component/
...
wasm_format:   module | component    # optional; default module. `component` emits a Component
                                     ...
                                     #   memory. `sets:` ARE supported (phase 3.2): `_all` lifts to
...
wac:           "path/to/wac"         # optional; defaults to $WAC, then wac in $PATH. The merge tool
...
resolved identically (config field → env var → `$PATH`).

`module` → `mergeModules`, `wasm-merge`, `$WASM_MERGE`:
...
`component` → `composeComponents`, `wac`, `$WAC`:
...
- **`sets:` are supported**, in a SECOND interface `sets` beside `matcher`, and
  ...
  constructor copies the input ONCE (a stateless per-position export measured
...
`wac` resolves as config `wac:` → `$WAC` → `$PATH`, the order the other two tools
...
- **wasm-tools** (external, Bytecode Alliance) — REQUIRED by `wasm_format: component` (`component/` wraps the core module with it); also used by several tests to validate emitted modules, where it is optional and the tests skip without it
...
/ `get_wasm_tools.sh` / `get_wac.sh`). A `module` build needs NONE of them to
