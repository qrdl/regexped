...
module-format counterparts so switching `wasm_format` changes no calling code.
...
Composition replaces merging: a component consumer is itself a component, and
`wac plug` links it to ours where `wasm-merge` links a module build.
