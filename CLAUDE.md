...
When `cfg.Sets` is non-empty, `CompileFile` (in `compile/set_emit.go`) replaces the bare `Compile` path: per-pattern bodies are emitted as before, then one function per declared set capability is appended (bucket dispatch built from `compile/set.go`, `compile/set_emit.go`, `compile/aho_corasick.go`). A set declares any of eight capabilities — `match`/`match_any`/`match_all` (anchored, whole input), `scan`/`scan_any`/`scan_all` (non-anchored, take a `from`), and `find`/`find_batch` (the only ones reporting positions and extents) — and the compiler emits only the machinery those need. `find`'s default body is per-pattern non-overlapping, matching Go `FindAllIndex`'s rule via a caller-owned gate array; `overlapping: true` selects the every-start-position enumeration instead. `find_batch` (`compile/set_batch.go`, plans/SETS.md §19) reports the same matches but fills the caller's buffer with several consecutive positions per call, resumed through an opaque i64 cursor; it is an INDEPENDENT capability with its own bodies, because a `find` caller may stop early and must not pay for speculation. See `plans/SETS.md` for the full design and `docs/sets.md` for the user-facing contract. Set stubs are generated by `generate/set_stub.go`.
...
│   │   │                      #   drives find AND find_batch, and cross-checks the two
...
│   └── sets.md                # Set composition: eight capabilities, YAML schema, gate array, output formats
└── examples/
    ├── README.md
    ├── Makefile
