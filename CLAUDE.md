...
│   ├── set_overlap_dp.go      # The overlapping answer cache: the backward sweep, its
│   │                          #   caller-owned header, and the COMMON cache emitters both
│   │                          #   find entries call. Only SERVING is per-entry, because
│   │                          #   `find` returns a position's TOTAL where the batch entry
│   │                          #   returns what it WROTE — which is why `find` does not
│   │                          #   forward into the batch entry to reach the cache
