...
When `cfg.Sets` is non-empty, `CompileFile` (in `compile/set_emit.go`) replaces the bare `Compile` path: per-pattern bodies are emitted as before, then one set-match function per set is appended (bucket dispatch built from `compile/set.go`, `compile/set_emit.go`, `compile/aho_corasick.go`). Set stubs are generated by `generate/set_stub.go`.
...
│   ├── set.go                 # Set composition: analyzePattern, CompileSet, frontend selection
│   ├── set_emit.go            # Set-match WASM emission (CompileFile, bucket dispatch, AC/Teddy/scalar frontends)
